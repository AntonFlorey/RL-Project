...
class Policy(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.actor_mean = nn.Sequential(
            layer_init(nn.Linear(state_dim, policy_layer_size)),
            nn.Tanh(),
            layer_init(nn.Linear(policy_layer_size, policy_layer_size)),
            nn.Tanh(),
            layer_init(nn.Linear(policy_layer_size, action_dim), std=0.01),
        )

        # Task 1: Implement actor_logstd as a torch tensor. Hint: when creating the torch tensor, remember to set the parameter device=device 
...
class PG(object):
    ...
    def update(self,):
        ...
        dis_rew = torch.reshape(h.discount_rewards(rewards, self.gamma), action_probs.shape)

        scaler = h.StandardScaler()
        scaler.fit(dis_rew)
        dis_rew_scaled = scaler.transform(dis_rew) # for 1 c)

        loss = -torch.mean(action_probs * (dis_rew_scaled - self.baseline)) # baseline for 1 b)
        loss.backward()
        self.optimizer.step()
    ...
    def get_action(self, observation, evaluation=False):
        ...
        act_logprob = torch.sum(probs.log_prob(action), axis=-1)
