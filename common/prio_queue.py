# This implementation is taken and adjusted from an open github repository
# link: https://github.com/Damcy/prioritized-experience-replay/blob/master/binary_heap.py
# 
...
class BinaryHeap(object):
    ...
    def update(self, priority, e_id):
        ...
        if e_id in self.e2p:
            ...
            success = self._insert(priority, e_id)
        return success

    def new_experience(self, priority, e_id):
        """
        add a new experience to the priority queue with given priority and buffer id
        :param priority: new priority value
        :param e_id: experience buffer id
        :return: bool
        """
        ...
        if e_id in self.e2p:
            p_id = self.e2p[e_id]
            self.priority_queue[p_id] = (priority+1, e_id)
            self.p2e[p_id] = e_id
